...
// Print out reservations (assuming you have implemented the toString method for the Reservation class)
    ...
    public ArrayList<Restaurant> findRestaurantsByType(String type) {
        ArrayList<Restaurant> topRestaurants = new ArrayList<>();
        for (int i = 0; i < restaurants.size() - 1; i++) {
            if (makeStringStandardForCheckingEquality(restaurants.get(i).getType()).equals(makeStringStandardForCheckingEquality(type))){
                topRestaurants.add(restaurants.get(i));
            }
        }
        return topRestaurants;
    }
    public ArrayList<Restaurant> findTopRestaurantsByOverallAvgByType(String type) {
//        ArrayList<Restaurant> sortedRestaurants = findRestaurantsByType(type);
//        Collections.sort(sortedRestaurants, Comparator.comparingDouble(Restaurant::getOverallAvg).reversed());
        ArrayList<Restaurant> topRestaurants = new ArrayList<>();
//        for (int i = 0; i < 6; i++) {
//                topRestaurants.add(sortedRestaurants.get(i));
//        }
    ...
    public ArrayList<Restaurant> findRestaurantsByLocation(String city, String country) {
        ArrayList<Restaurant> sortedRestaurants = restaurants;
        ArrayList<Restaurant> topRestaurants = new ArrayList<>();
        for (int i = 0; i < sortedRestaurants.size() - 1; i++) {
            if (!makeStringStandardForCheckingEquality(city).equals("")){
                if(!makeStringStandardForCheckingEquality(country).equals("")){
                    String restaurantCountry = makeStringStandardForCheckingEquality(sortedRestaurants.get(i).getAddress().getCountry());
                    String restaurantCity = makeStringStandardForCheckingEquality(sortedRestaurants.get(i).getAddress().getCity());
                    if(country.equals(restaurantCountry) & city.equals(restaurantCity)) {
                        topRestaurants.add(sortedRestaurants.get(i));
                    }
                } else {
                    String restaurantCity = makeStringStandardForCheckingEquality(sortedRestaurants.get(i).getAddress().getCity());
                    if (city.equals(restaurantCity)){
                        topRestaurants.add(sortedRestaurants.get(i));
                    }
                }
            } else {
                if(!makeStringStandardForCheckingEquality(country).equals("")){
                    i = sortedRestaurants.size() + 1;
                } else {
                    String restaurantCountry = makeStringStandardForCheckingEquality(sortedRestaurants.get(i).getAddress().getCountry());
                    if(country.equals(restaurantCountry)){
                        topRestaurants.add(sortedRestaurants.get(i));
                    }
                }
            }
    ...
    public ArrayList<Restaurant> findTopRestaurantsByOverallAvgByLocation(String city, String country) {
        ArrayList<Restaurant> sortedRestaurants = findRestaurantsByLocation(city, country);
        Collections.sort(sortedRestaurants, Comparator.comparingDouble(Restaurant::getOverallAvg).reversed());
        ArrayList<Restaurant> topRestaurants = new ArrayList<>();
        for (int i = 0; i < 6; i++) {
            topRestaurants.add(sortedRestaurants.get(i));
    ...
    public String makeStringStandardForCheckingEquality(String str){
        str = str.replaceAll("[^a-zA-Z]", "");
        str = str.toLowerCase();
        return str;
    }
    public ArrayList<Restaurant> findTopRestaurantsByUserLocation(String username){
        User user = getUserByUsername(username);
        String userCity = makeStringStandardForCheckingEquality(user.getAddress().getCity());
        String userCountry = makeStringStandardForCheckingEquality(user.getAddress().getCountry());
        ArrayList<Restaurant> sortedRestaurants = restaurants;
        Collections.sort(sortedRestaurants, Comparator.comparingDouble(Restaurant::getOverallAvg).reversed());
        ArrayList<Restaurant> topRestaurants = new ArrayList<>();
        for (int i = 0; i < sortedRestaurants.size() - 1; i++){
            String restaurantCountry = makeStringStandardForCheckingEquality(sortedRestaurants.get(i).getAddress().getCountry());
            String restaurantCity = makeStringStandardForCheckingEquality(sortedRestaurants.get(i).getAddress().getCity());
            if(userCountry.equals(restaurantCountry) & userCity.equals(restaurantCity)){
                topRestaurants.add(sortedRestaurants.get(i));
